...
import TableOne from '../../components/Tables/Consultings/TablesFiles';
import Breadcrumb from '../../components/Breadcrumbs/Breadcrumb';
import Folder from '../../components/Folders/Consultings/Folder';
import axios from 'axios';
import { useDispatch, useSelector } from 'react-redux';
import { getFiles, getFolders } from '../../redux/Consultings/ConsultingsSlice';
...
const Consultings: React.FC = () => {
    const [parentId, setParentId] = useState<string>("null");
    const dispatch = useDispatch();

    const { folders,mainFolder,fileInfo,files } = useSelector((state: RootState) => ({
        folders: state.Consultings.folders,
        mainFolder:state.Consultings.mainFolder,
  ...
  console.log(mainFolderName);  
    ...
    return (
        <>
            ...
            <Folder folders={folders || []} title={"Consultings's Folders"}  /> 
...
{ files &&
    ...
    <div className="flex justify-center items-center min-h-screen">
        <div className="rounded-xl bg-slate-200 p-6 text-center shadow-xl max-w-lg dark:bg-boxdark">
            ...
            <a href="#" className="text-gray-900 font-bold text-lg mb-2 hover:text-indigo-600 inline-block mt-8">
                Path of file in Server
            </a>
            <p className="text-gray-700 text-sm">{fileInfo.path.pathString}</p>
        </div>
    </div>
)}
